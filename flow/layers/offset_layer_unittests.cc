...
using OffsetLayerTest = LayerTest;

#ifndef NDEBUG

TEST_F(OffsetLayerTest, PaintBeforePrerollDies) {
  SkPath child_path;
  child_path.addRect(5.0f, 6.0f, 20.5f, 21.5f);
  auto mock_layer = std::make_shared<MockLayer>(child_path, SkPaint());
  auto layer = std::make_shared<OffsetLayer>(0.0f, 0.0f);
...
TEST_F(OffsetLayerTest, Identity) {
  SkPath child_path;
  child_path.addRect(5.0f, 6.0f, 20.5f, 21.5f);
 ...
 // EXPECT_EQ(mock_layer->parent_mutators(), std::vector({Mutator(0.0f, 0.0f)}));
...
TEST_F(OffsetLayerTest, Simple) {
  ...
  auto mock_layer = std::make_shared<MockLayer>(child_path, SkPaint());
  auto layer = std::make_shared<OffsetLayer>(2.5f, 2.5f);
  layer->Add(mock_layer);
